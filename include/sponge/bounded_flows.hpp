#ifndef SPONGE_BOUNDED_FLOWS_HPP
...
namespace sponge
{
	...
	public:
		...
		optional<Flow> maxf(int s0,int t0)
		{
			base_type::link(t0,s0,inf<Flow>);
			if(!feasible())return nullopt;
			s=s0,t=t0;
			base_type::maxflow=0;
			return this->mf();
		}
		optional<Flow> minf(int s0,int t0)
		{
			base_type::link(t0,s0,inf<Flow>);
			int id=e;
			if(!feasible())return nullopt;
	...
	public:
		...
		optional<Cost> mcf(int s0,int t0)
		{
