...
namespace sponge
{
	...
	public:
		...
		void apply_path_tag(int x,t_cr t)
		{
			if(!x)return;
		...
		void erase(int x)
		{
			...
			{
				int y=ls(x);
				push_down<satt_rake>(y);
				while(rs(y))y=rs(y),push_down<satt_rake>(y);
				splay<satt_rake>(y,x);
				setf(rs(x),y,1);
				setf(y,fa(x),2);
				push_up<satt_rake>(y);
				push_up<satt_compress>(fa(x));
			}
		...
		void splice(int x)
		{
			splay<satt_rake>(x);
			int y=fa(x);
			splay<satt_compress>(y);
			push_down<satt_rake>(x);
			if(rs(y))swap(fa(ms(x)),fa(rs(y))),swap(ms(x),rs(y));
			else erase(x);
			push_up<satt_rake>(x);
			push_up<satt_compress>(y);
		}
		int access(int x)
		{
			splay<satt_compress>(x);
			int z=x;
			if(rs(x))
			{
				int y=new_node();
		...
		void set_value(int x,s_cr v=s_id())
		{
			value(x)=v;
			push_up<satt_compress>(x);
		}
		...
		{
			access(x);
			while(ls(x))push_down<satt_compress>(x),x=ls(x);
			splay<satt_compress>(x);
			return x;
		}
		...
		int cut(int x)
		{
			access(x);
			int y=ls(x);
			while(rs(y))push_down<satt_compress>(y),y=rs(y);
			ls(x)=fa(ls(x))=0;
			push_up<satt_compress>(x);
			return y;
		}
		...
		void update_path(int x,int y,t_cr t)
		{
			int root=find(x);
			expose(x,y);
			apply_path_tag(y,t);
		...
		void update_vertex(int x,t_cr t)
		{
			update_path(x,x,t);
		}
		void update_subtree(int x,t_cr t)
		{
			int root=find(x);
			access(x);
