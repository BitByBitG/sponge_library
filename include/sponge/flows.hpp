...
namespace sponge
{
	...
	public:
		...
		bool bfs()
		{
			...
			while(!dq.empty())
			{
				...
				{
					int y=son[i];
					if(cap[i]>eps<Flow>&&dis[y]==inf<int>)
					{
						dq.push_back(y);
						dis[y]=dis[x]+1;
						if(y==t)return 1;
		...
		Flow dfs(int x,Flow flow)
		{
			if(x==t)return flow;
			Flow res=0;
			for(int& i=cur[x];i&&flow>eps<Flow>;i=next[i])
			{
				int y=son[i];
				if(cap[i]>eps<Flow>&&dis[y]==dis[x]+1)
				{
					Flow k=dfs(y,min(flow,cap[i]));
					if(k<=eps<Flow>)dis[y]=inf<int>;
					cap[i]-=k;
					cap[i^1]+=k;
					res+=k;
					flow-=k;
		...
		void dinitz()
		{
			while(bfs())
			{
				Flow flow=dfs(s,inf<Flow>);
				if(flow<=eps<Flow>)break;
				maxflow+=flow;
			}
		}
		Flow mf()
	...
	class pd_graph
	{
		static_assert(is_integral_v<Flow>,"Flow must be an integral type");
	public:
		using flow_type=Flow;
		...
		void spfa()
		{
			...
			while(!dq.empty())
			{
				...
				{
					int y=son[i];
					if(cap[i]>0&&h[y]>h[x]+cost[i]+eps<Cost>)
					{
						h[y]=h[x]+cost[i];
						if(!vis[y])
						{
		...
		bool dijkstra()
		{
			...
			while(!pq.empty())
			{
				...
				for(int i=head[x];i;i=next[i])
				{
					...
					if(cap[i]>0&&dis[y]>dis[x]+w+eps<Cost>)
					{
						dis[y]=dis[x]+w;
						pre_v[y]=x;
						pre_e[y]=i;
